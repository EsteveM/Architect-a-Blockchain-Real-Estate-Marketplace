...
contract('TestSolnSquareVerifier', accounts => {
    ...
    describe('Test SolnSquareVerifier', function () {
        beforeEach(async function () { 
            // EMG - The address of the Verifier contract must be provided as a parameter
            // to the SolnSquareVerifier contract
        ...
        it('Test if a new solution can be added for contract', async function () { 
            let tokenId = 1;
            await this.contract.registerSolution(
            ...
            let submitted = await this.contract.solutionHasBeenSubmitted(
                                                    verifierInput.proof.a,
                                                    verifierInput.proof.a_p,
                                                    verifierInput.proof.b,
                                                    verifierInput.proof.b_p,
            ...
            assert.equal(submitted, true, "Solution should have already been submitted");
        })
        ...
        // Test that a repeated solution cannot be used - SolnSquareVerifier
        it('Test if the same solution can be used twice', async function () {
        /*    let tokenId = 1;
            let owner = await this.contract.ownerOf(tokenId, {from: account_one});
            assert.notEqual(account_two, owner, "Token owner should not be account"); */
            
        // The solution is used for the first time
            let tokenId = 1;
            await this.contract.mintNFT(
                                    verifierInput.proof.a,
                                    verifierInput.proof.a_p,
            ...
            let owner = await this.contract.ownerOf(tokenId, {from: account_one});
            assert.equal(account_two, owner, "It should be possible to use a solution for the first time");
            
            // An attempt is made to use the same solution twice
            tokenId = 2;
            let reverted = false;
            try 
            {
